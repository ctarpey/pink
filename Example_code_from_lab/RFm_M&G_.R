...
plot.pseudo.y <- function(LG, plot){
  ...
  if (plot == "Y"){
    ...
    #
    plot(NULL, pch=19, col="red", xlab="cM", ylab="RF (phase-shift)", xlim=c(0,160), ylim=c(0,1), las=1, main=paste("LG",LG))
...
pdf(file = "G:/Analysis/Mapping/AllHaps/Centromeres/plots/05_LG1")
...
setwd("G:/Analysis/Mapping/AllHaps" )


#Read in map with renamed LGs according to sockeye map & Gyno Ho values:
mapped_loci_PSV <- read.csv(file = 'Pink_map_loci_data.csv', sep = ',')
...
for (j in LGs){
  ...
  for (i in 1:length(mapped_loci_PSV$LG_lepmap)){  
    
    if (mapped_loci_PSV$LG_lepmap[i]==j){
