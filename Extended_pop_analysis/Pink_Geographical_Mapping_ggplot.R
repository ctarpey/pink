### Creating Geographical Maps in GGPlot
###     
...
OG_data <- my_data[(my_data$Location != "Skeena") & (my_data$Location !=  "Susitna"),] 
dim(OG_data)

# subset the  split set of data to get OG samples only
Susitna <- my_data[(my_data$Location ==  "Susitna"),] 
dim(Susitna)

# subset the Susitna and Asia
Asia_Susitna <- my_data[(my_data$CONTINENT=="Asia") | (my_data$Location=="Norton Sound") | (my_data$Location ==  "Susitna"),] 
dim(Asia_Susitna)
...
Susitna_long<- Susitna$Longitude
Susitna$Long_360 <- make360(Susitna_long)
head(Susitna)


Asia_Susitna_long<- Asia_Susitna$Longitude
Asia_Susitna$Long_360 <- make360(Asia_Susitna_long)
head(Asia_Susitna)
...
ggplot() +
  ...
  geom_polygon(data = Japan, aes(x=long, y = lat, group = group), fill="grey17", alpha=0.15) +
  geom_polygon(data = China, aes(x=long, y = lat, group = group), fill=" grey17", alpha=0.15) +
  geom_point(data=Asia_Susitna, aes(x=Long_360, y=Latitude), size = 4, alpha = 0.8, color = "black" ) +
  theme(panel.background = element_rect(fill = "white"), 
        panel.grid.major = element_line(colour = NA), 
        axis.text=element_text(size=12),
        axis.title =element_text(size=14),
  ...
  labs(x = "Longitude", y = "Latitude") +
  coord_map(xlim= c(135, 245),  ylim = c(39,66)) +
  geom_text_repel(data= Asia_Susitna, aes(x=Long_360, y=Latitude, label = Location), size=4, box.padding= 0.5, segment.colour = NA) 
