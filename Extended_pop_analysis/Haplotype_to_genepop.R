### Convert a Haplotype file with the Stacks formatting to a Genepop file
###   This is  a reformatting of the script that Mark Ravinet wrote
...
#This code requires the haplotype file that was filtered for paralogs and individuals in ThirdPinkFiltering.R
...
#input the haplotype file
infile <- read.table("Z:/WORK/TARPEY/Exp_Pink_Pops/FilteringGenotypes/HDPlot/Haplotypes_with465ins_singletons.txt", header=TRUE)
infile[1:5,1:5]
...
# First sweep through haplo for any individuals with > 2 alleles
haplo <- apply(infile, 2, function(x){
  sapply(x, function(y){
    if(str_count(y, "/") > 1){
      y <- "-"
    } else{
