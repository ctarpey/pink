### Plot linkage groups of a map individually in a big grid 
###    Written by Morten Limborg  
### Carolyn Tarpey | January  2016 
### ---------------------------------------


### this has whole sections that are blocked out because I dont have the data to 
### actually plot them. The resulting plots are therefore, pretty bland.
...
Map <- read.csv(file = 'Pink_map_verti.txt', sep = '\t') #Read in linkage map

names(Map)
#Boot_results <- read.csv(file = 'Kokanee_Bootstrap_results.csv', sep = ',')

#PSV map LG names:
LGs <- as.character(unique(Map$LG_lepmap))
...
#pop.comparisons <- colnames(Map)[7:21]
...
pop.comparisons <- colnames(Map)[3:10]
data_to_plot <- pop.comparisons[4] #Defining pop comparison to plot:
file = paste(data_to_plot, "_test.pdf", sep="") #Name of output file
...
LGs_Y_axis <- LGs[c(1,7,13,19,25)] #LGs for which y-axes will be drawn: These are the most right and bottom?
LGs_X_axis <- LGs[c(21:27)]        #LGs for which x-axes will be drawn: these are the most top? 
...
for (j in LGs){
  ...
  ##################################
  #j <- c("1")
  LG_j_loci <- which(Map$LG_lepmap == j)
  LG_j_Map <- Map[LG_j_loci,]
  ...
  plot(x=NULL, y=NULL, xlim=c(-5,160), axes=F,ann=F, ylim=ylim, xaxs="i",yaxs="i")
  mtext(text=paste("LG #", j, sep=""), side=3, line=0.3, cex=0.65)
  box(col="gray80")
