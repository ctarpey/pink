...
###    Written by Morten Limborg  this is the original from Morten
###    unedited by me, for reference to the version that I have edited
...
#setwd("C:/Users/moli/Google Drev/Morten arbejde/Sockeye vs Kokanee/Paper 1-2/Mapping/Maps/Kokanee_map_Rproj/Final_Map")
#setwd("C:/Users/moli/Google Drev/Morten arbejde/Sockeye vs Kokanee/Paper 1-2/Text/Paper 3/Examples_other studies/Limborg_haploid pink map")
#setwd("C:/Users/moli/Google Drev/Morten arbejde/Sockeye vs Kokanee/Paper 1-2/Text/Paper 3/Examples_other studies/Milller_mykiss_map")
#setwd("C:/Users/moli/Google Drev/Morten arbejde/Sockeye vs Kokanee/Paper 1-2/Text/Paper 3/Examples_other studies/Chutiman_et_al._2011_Barley_map")
setwd("C:/Users/moli/Google Drev/Morten arbejde/Sockeye vs Kokanee/Paper 1-2/Text/Paper 3/Examples_other studies/McKinney_Chinook_diplod_data_map/phased genotypes")

plot.pseudo.y <- function(LG, plot){
  
  ##Read in map with phased genotypes:
#  Map <- as.matrix(read.csv(file = 'Map_phased_genotypes_haploids_for_R.csv', sep = ',')) #Read map w. haploid genotypes
#  Map <- as.matrix(read.csv(file = 'Miller_mykiss_map.csv', sep = ',')) #Read map w. haploid genotypes
#  Map <- as.matrix(read.csv(file = 'Pink_female_mapX05_phased_R.csv', sep = ',')) #Read map w. haploid genotypes
#  Map <- as.matrix(read.csv(file = 'barley_dh_RADmap_for_R.csv', sep = ',')) #Read map w. haploid genotypes
  Map <- as.matrix(read.csv(file = 'KUWDip_F13_M7_Sire_MSTMapPhased_LGpos.csv', sep = ',')) #Read map w. haploid genotypes
...
is.matrix(Map)
  ...
  if (plot == "Y"){
  plot(NULL, pch=19, col="red", xlab="cM", ylab="RF (phase-shift)", xlim=c(0,360), ylim=c(0,1.2), las=1, main=paste("LG",LG))
...
write.csv(t(RFm_matrix), file="barley_RFm.csv")
